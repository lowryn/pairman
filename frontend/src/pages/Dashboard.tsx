...
const PROTOCOL_COLOURS: Record<string, { badge: string; bar: string }> = {
  Matter:    { badge: 'bg-violet-100 text-violet-700', bar: 'bg-violet-500' },
  HomeKit:   { badge: 'bg-orange-100 text-orange-700', bar: 'bg-orange-500' },
  'Z-Wave':  { badge: 'bg-green-100  text-green-700',  bar: 'bg-green-500'  },
  Zigbee:    { badge: 'bg-yellow-100 text-yellow-700', bar: 'bg-yellow-500' },
  WiFi:      { badge: 'bg-blue-100   text-blue-700',   bar: 'bg-blue-500'   },
  Bluetooth: { badge: 'bg-sky-100    text-sky-700',    bar: 'bg-sky-500'    },
  Thread:    { badge: 'bg-teal-100   text-teal-700',   bar: 'bg-teal-500'   },
  Other:     { badge: 'bg-gray-100   text-gray-600',   bar: 'bg-gray-400'   },
  Unknown:   { badge: 'bg-gray-100   text-gray-600',   bar: 'bg-gray-400'   },
}
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <Link to="/devices" className="bg-white border rounded-xl p-4 hover:shadow-md transition-shadow">
          <p className="text-3xl font-bold">{total_devices}</p>
          <p className="text-sm text-gray-500 mt-1">Devices</p>
        </Link>
        <Link to="/settings" className="bg-white border rounded-xl p-4 hover:shadow-md transition-shadow">
          <p className="text-3xl font-bold">{total_homes}</p>
          <p className="text-sm text-gray-500 mt-1">Homes</p>
        </Link>
        <div className="bg-white border rounded-xl p-4">
          <p className="text-3xl font-bold">{total_rooms}</p>
      ...
      {total_devices === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3 bg-white border rounded-xl p-4">
            <h2 className="font-semibold mb-4">By Home</h2>
            <ul className="space-y-4">
              {by_home.map(home => (
                <li key={home.home_id}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium">{home.home_name}</span>
                    <span className="text-sm text-gray-500">{home.count}</span>
                  </div>
                  ...
                  {home.rooms.filter(r => r.count > 0).length > 0 && (
                    <ul className="pl-3 space-y-1 border-l-2 border-gray-100">
                      {home.rooms.filter(r => r.count > 0).map(room => (
                        <li key={room.room_id} className="flex justify-between text-xs text-gray-500">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            {by_protocol.length > 0 && (
              <div className="bg-white border rounded-xl p-4">
                <h2 className="font-semibold mb-4">By Protocol</h2>
                ...
                <ul className="space-y-2">
                  {by_device_type.map(({ device_type, count }) => (
                    <li key={device_type} className="flex justify-between text-sm">
                      <span className="text-gray-700">{device_type}</span>
